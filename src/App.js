...
class App extends React.Component {
  constructor(props) {
    ...
    this.setEffects = [
      createSetEffect(1, 'Absolute Unit (2) ', 2, [2, 13, 14, 36, 37, 45, 46, 51], 'Players take 30% less damage from monsters, with 50% of post-reduction damage reflected back to the attacker.'),
      createSetEffect(1, 'Absolute Unit (4) ', 4, [2, 13, 14, 36, 37, 45, 46, 51], 'Players take 50% less damage from monsters, with 100% of post-reduction damage reflected back to the attacker.'),
      createSetEffect(2, 'The Alchemist', 4, [7, 12, 16, 18, 21, 25, 41], 'When cooking food, making jugs of wine, cleaning grimy herbs, or mixing potions, all items in the player\'s inventory will be processed at once and reward the full amount of experience.'),
      createSetEffect(3, 'Chain Magic (2)', 2, [11, 19, 27, 34, 47, 50, 52], 'Players have a 30% chance to perform another magic attack after a successful hit with magic.'),
      createSetEffect(3, 'Chain Magic (3)', 3, [11, 19, 27, 34, 47, 50, 52], 'Players have a 60% chance to perform another magic attack after a successful hit with magic.'),
      createSetEffect(4, 'The Craftsman', 4, [14, 17, 20, 30, 31, 35], 'When smelting ores, smithing bars and cannonballs, crafting with leather, glass, uncut gems, pottery, battlestaves, spinning flax and wool, fletching logs and cutting bolt tips, all items in the player\'s inventory will be processed at once and reward the full amount of experience.'),
      createSetEffect(5, 'Double Tap (2)', 2, [2, 9, 30, 33, 35, 42, 48], 'Players have a 30% chance to perform another ranged attack after a successful hit with ranged.'),
      createSetEffect(5, 'Double Tap (3)', 3, [2, 9, 30, 33, 35, 42, 48], 'Players have a 60% chance to perform another ranged attack after a successful hit with ranged.'),
      createSetEffect(6, 'Drakan\'s Touch (2)', 2, [10, 22, 29, 44, 48, 50, 52], 'Successful hits have a 50% chance of restoring the player\'s hitpoints by 5% of the damage dealt.'),
      createSetEffect(6, 'Drakan\'s Touch (4)', 4, [10, 22, 29, 44, 48, 50, 52], 'Successful hits have a 50% chance of restoring the player\'s hitpoints by 10% of the damage dealt. '),
      createSetEffect(7, 'Endless Knowledge', 4, [0, 1, 15, 20, 27, 28, 47], 'Players receive an arcane grimoire from The Sage, which allows the ability to freely switch spellbooks anywhere. '),
      createSetEffect(8, 'Fast Metabolism', 3, [3, 17, 19, 22, 32, 51], 'Hitpoint regeneration is increased by x4.'),
      createSetEffect(9, 'Greedy Gatherer (3)', 3, [7, 8, 21, 24, 26, 32, 39], 'Players will receive double the resources (with full experience) from Fishing, Woodcutting, Mining, and Farming.'),
      createSetEffect(9, 'Greedy Gatherer (4)', 4, [7, 8, 21, 24, 26, 32, 39], 'Players will receive triple the resources (with full experience) from Fishing, Woodcutting, Mining, and Farming.'),
      createSetEffect(10, 'Knife\'s Edge (2)', 2, [4, 5, 23, 24, 29, 37, 41, 49], 'Players will deal 0.3% additional damage for every hitpoint missing.'),
      createSetEffect(10, 'Knife\'s Edge (4)', 4, [4, 5, 23, 24, 29, 37, 41, 49], 'Players will deal 0.6% additional damage for every hitpoint missing.'),
      createSetEffect(11, 'Last Recall', 5, [9, 10, 15, 18, 31, 34, 36, 40, 43, 45], 'Players receive a crystal of memories from The Sage, which allows them to teleport themselves back to the location where their most recent teleport occurred, with some caveats regarding its usage.'),
      createSetEffect(12, 'Personal Banker (2)', 2, [0, 6, 11, 26, 38, 42], 'Resources obtained from Fishing, Woodcutting, Mining, and Farming have a 50% chance to be deposited directly to the player\'s bank.'),
      createSetEffect(12, 'Personal Banker (4)', 4, [0, 6, 11, 26, 38, 42], 'Resources obtained from Fishing, Woodcutting, Mining, and Farming are deposited directly to the player\'s bank.'),
      createSetEffect(13, 'Trailblazer', 3, [1, 5, 8, 16, 38, 43, 49], 'Players receive a portable waystone from The Sage, which can teleport to other waystones.'),
      createSetEffect(14, 'Twin Strikes (2)', 2, [3, 4, 13, 23, 40, 44, 46], 'Players have a 30% chance to perform another melee attack after a successful hit with melee.'),
      createSetEffect(14, 'Twin Strikes (3)', 3, [3, 4, 13, 23, 40, 44, 46], 'Players have a 60% chance to perform another melee attack after a successful hit with melee. '),
      createSetEffect(15, 'Unchained Talent', 3, [6, 12, 25, 28, 33, 39], 'All non-combat skills are permanently boosted by +8. This boost is not limited to unlocked skills.')
    ];
      ...
      <div>
        <div id='topbar'>
          <div className='topbar-row'>
            {/*<div className={'top-bar-button ' + (this.state.currentTab === 'write' ? 'active' : '')}  onClick={() => this.handleChange('currentTab', 'write')}>Write</div>
            <div className={'top-bar-button ' + (this.state.currentTab === 'entries' ? 'active' : '')}  onClick={() => this.handleChange('currentTab', 'entries')}>Journal</div>*/}
